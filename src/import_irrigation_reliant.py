...
To use this, you want to set the code to 5 bins in Params.ods, run import_irrigation_reliant.py, then set it back to 1 
bin and run upsample_irrigation.py.
...
import rasterio
from utilities import makeGrid, rebinCumulative
...
if platform == "linux" or platform == "linux2":
    # this is to ensure Morgan's computer doesn't crash, but set it if you have a small amount of ram available.
    import resource

    rsrc = resource.RLIMIT_AS
    resource.setrlimit(rsrc, (int(5e9), int(5e9)))  # no more than 5 gb

# load the params from the params.ods file into the params object
aquastat = pd.read_csv("../data/aquastat.csv", index_col=False)
...
# total area
areadata = rasterio.open("../data/gmia_v5_aei_ha.asc")
# hectares
gwdata = rasterio.open("../data/gmia_v5_aeigw_pct_aei.asc")
# surface water
swdata = rasterio.open("../data/gmia_v5_aeisw_pct_aei.asc")
...
five_minute = 5 / 60  # in degrees
cell_size = five_minute * 10

# we ignore the last latitude cell
lats = np.linspace(-90, 90 - cell_size, np.floor(180 / cell_size).astype("int"))
lons = np.linspace(-180, 180 - cell_size, np.floor(360 / cell_size).astype("int"))
...
areaBinned = rebinCumulative(areaArrResized, sizeArray)
areaBinnedReoriented = np.fliplr(np.transpose(areaBinned))
swBinned = rebinCumulative(swArrResizedFiltered, sizeArray)
swBinnedReoriented = np.fliplr(np.transpose(swBinned))
gwBinned = rebinCumulative(gwArrResizedFiltered, sizeArray)
...
geometry_sjoin = gpd.points_from_xy(
    df.lons + cell_size / 2.0, df.lats + cell_size / 2.0
)
...
def estimate_reliant_scheme_sw(
    ...
    if ~np.isnan(sw_area) and sw_area > 0:
        ...
        # remove unreliant surface water irrigation area from consideration, and assume remaining sprinkler, localized,
        # and surface irrigation are evenly split between ground and surface water irrigated area
        ...
        if nonsw_area == 0:
            ...
            # now, even splitting of each, but will need to multiply out by multiplier to get back to original total
            # fraction
...
def estimate_scheme_reliance(
    ...
    if abs(scheme_total) - 1 > abs(total_irrigated_area_zeroed) or abs(
        ...
        # in this case we don't have good direct estimate regarding the fraction of the scheme that is reliant. We'll
        # guess all remaining area is electrified if spate or lowland are listed explicitly,
        ...
        if localized_area_zeroed + sprinkler_area_zeroed == 0:
            if localized_area == 0 or sprinkler_area == 0:
                ...
                spate_area_zeroed + lowland_area_zeroed + surface_area_zeroed
                > max_area / 2
            ):
                reliant_scheme = (
                    max_area
...
for countryIndex in set(pointInPolys.index_right.values):
    ...
    scheme_total_arr = np.array(
        [surface_area, sprinkler_area, localized_area, spate_area, lowland_area]
    )
...
grid.to_csv("../data/Irrigation.csv")
