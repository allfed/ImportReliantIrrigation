...
if platform == "linux" or platform == "linux2":
	...
	resource.setrlimit(rsrc, (5e9, 5e9))#no more than 5 gb
...
lats = np.linspace(-90, 90 - cell_size, \
			   np.floor(180 / cell_size).astype('int'))
lons = np.linspace(-180, 180 - cell_size, \
			   np.floor(360 / cell_size).astype('int'))
...
geometry_sjoin = gpd.points_from_xy(df.lons+cell_size/2., df.lats+cell_size/2.)
